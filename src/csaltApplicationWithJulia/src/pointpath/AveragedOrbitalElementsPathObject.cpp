...
AveragedOrbitalElementsPathObject::AveragedOrbitalElementsPathObject(Integer n) : 
    n(n)//, taus(JLvector(n)), ws(JLvector(n)), x(JLvector(6)), dx(JLvector(6))
{
    // Handel Julia exception from allocation of JLvector(s)
    //handle_julia_exception();

    // Set qauss quadrature weights and nodes by calling FastGaussQuadrature.jl
    // First get pointer to function
    //jl_function_t* set_gq   = jl_get_function(jl_main_module, "fill_gausslegendre!");

    // The set function arguments
    //jl_value_t* args[]      = {(jl_value_t*) taus.GetJuliaVector(), 
    //                           (jl_value_t*) ws.GetJuliaVector(), 
    //                           jl_box_int64(n)};

    // Call function with 3 args and handle any exceptions
    //handle_jl_call(set_gq, args, 3);
}
